...
const DeposerCours = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!file) throw new Error('Veuillez ajouter un fichier de cours');
      ...
      if (!selectedFormationName || !selectedCategoryId) {
        throw new Error('Informations de formation incomplètes');
      }
      ...
      // Use categorieId for the course path
      const courseRef = ref(db, `cours/${selectedCategoryId}/${selectedFormation}/${semester}/${courseId}`);
  ...
  return (
    <Container>
      <FormContainer>
        <Header>
          <Title>Déposer un Cours</Title>
        </Header>

        {selectedFormationName && (
          <SelectedFormation>
            Formation sélectionnée: <strong>{selectedFormationName}</strong>
            {selectedCategoryId && <span> (Catégorie: {formations.find((f) => f.id === selectedFormation)?.categoryName})</span>}
          </SelectedFormation>
        )}
        ...
        <StyledForm onSubmit={handleSubmit}>
            ...
            <Select value={selectedFormation} onChange={handleFormationChange} required>
              ...
              {formations.map((formation) => (
                <option key={formation.id} value={formation.id}>
                  {formation.intitule} {formation.specialite ? `- ${formation.specialite}` : ''} ({formation.categoryName})
                </option>
              ))}
            </Select>
          </FormGroup>
          ...
          <FormGroup>
            ...
            <FileHint>Formats acceptés: PDF, DOC, DOCX, PPT, PPTX (Taille max: 10MB)</FileHint>
          </FormGroup>
